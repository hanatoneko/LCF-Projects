function[m,n,cbar,Abar,Bbar,bbar,Ubar,Cbar,gbar,Fbar]=build_simplev(C,D,r,a,U,theta)
...
%%% number of drivers' menus a request can be added to  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for i=1:m
    for j=1:n
        Abar(crow+i,m*n+i+m*(j-1))=-1;
    end
    Bbar(crow+ i,:)=zeros(1, (m+1)*n);
    bbar=[bbar, -a];
end
crow = crow + m;
...
for j=1:n
   crow=0;
   for i=1:m
      Fbar(3+(j-1)*q+crow, m*n + (j-1)*m+i)=1; 
      crow=crow+1;
   end
end
