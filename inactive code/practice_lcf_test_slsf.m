%%% this code is basically the same as regression_test with extra code like
%%% double test removed, and the user can decide if the best solution is
%%% guessed by doing minitest on the slsf solns, the lcf solns, or both
m=20;
n=20;
s=10;
sfixed=10;
...
if iterative==1
    ...
    %numtests=iterations;
        ...
        ptruebest=zeros(m,n);
     if perfiter1 == 1
         iter1objave = zeros(numtests,trials);
         iter1rejs = zeros(numtests,trials);
         iter1prof = zeros(numtests,trials);
         iter1unhap = zeros(numtests,trials);
     end
...
for t=1:trials
 ...
 runtimes(ell,t)=toc;
  ...
  if fixedmenu==1
    ...
    if iterative==1
             if perfiter1 == 1
                 %first iteration performance test
                 [testyhat,testscenprobs,testscenbase,testscenexp]=indepy_generate_scens(x.*ptrue,5000,0);
                  [objave1est,objsd,dupave1,dupsd,rejave,rejsd,aveassign1,objvals1,vfull1,zfull1,wfull1]=sample_cases_performance_indepy(Chat-u-alphavals,D,zeros(m,1),q,x,ptrue,5000,testyhat,testscenprobs); 
                 ...
                 iter1objave(ell,t) = objave1est;
                 iter1rejs(ell,t) = rejave;
                 iter1prof(ell,t) = avetotprofit;
                 iter1unhap(ell,t) = dupave1;
             end
...
%               end
            ...
            runtimes(ell,t)=runtimes(ell,t)+toc;
              oldmenu=x;
               ...
               if testslsfsolns == 1
                  %slsf minitest
                  [testyhat,testscenprobs,testscenbase,testscenexp]=indepy_generate_scens(x.*yprobs,minitestsize,0);
                  [objave1est,objsd,dupave1,dupsd,rejave,rejsd,aveassign1,objvals1,vfull1,zfull1,wfull1]=sample_cases_performance_indepy(C,D,r,q,x,yprobs,minitestsize,testyhat,testscenprobs); 
                  if guessbestmetric=='o'
                      compareest=objave1est;
                  else
                      compareest=-rejave;
                  ...
                  if compareest>bestest(ell,t)            
                      bestest(ell,t)=compareest 
                       guessbestiter(ell,t)=k + 0.5
                       xbest=x;
                       ubest=newcomp-alphavals;
                       ptruebest=yprobs;
                  end
              end
...
%%% calculate averages and display soln info
if iterative==1
    if cumperf==1
        
        lcfiterbetter=size(find(returnedobjs>slsfobjs),1)/(numtests*trials)
         lcfiterbetter1=size(find(iter1objave>slsfobjs),1)/(numtests*trials)
        lcfiterworse=size(find(returnedobjs<slsfobjs),1)/(numtests*trials)
        lcfiterworse1=size(find(iter1objave<slsfobjs),1)/(numtests*trials)
        avepercbetter=mean(mean((returnedobjs./slsfobjs)-ones(numtests,trials)))
        avepercbetter1=mean(mean((iter1objave./slsfobjs)-ones(numtests,trials)))
        lcfrejbetter=size(find(lcfrejs<slsfrejs),1)/(numtests*trials)
        lcfrejbetter1=size(find(iter1rejs<slsfrejs),1)/(numtests*trials)
        lcfrejworse=size(find(lcfrejs>slsfrejs),1)/(numtests*trials)
        lcfrejworse1=size(find(iter1rejs>slsfrejs),1)/(numtests*trials)
        avepercrejbetter=mean(mean(ones(numtests,trials)-(lcfrejs./slsfrejs)))
        avepercrejbetter1=mean(mean(ones(numtests,trials)-(iter1rejs./slsfrejs)))
          ...
          %avepercbetter=mean(mean(returnedobjs))/mean(mean(slsfobjs))-1              
           averuntime=mean(mean(runtimes))
            avelcfrejs=mean(mean(lcfrejs))
             ...
             avelcfunhap1=mean(mean(iter1unhap))
            ...
            avelcfavetotprofit1=mean(mean(iter1prof))
