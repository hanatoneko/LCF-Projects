function[obj,gap,x,vf,w,y,p,ptrue,u,psi]=comp_model_no_v_w_scens(Chat,theta,lwr,q,s,fyhat,fscenprobs,fare,alpha,beta,buffer,mincomp,timelim,gap,displ)

%%%%%%%%%%%%%%%%%%%% NOT A GOOD FORMULATION SINCE YOU HAVE FIXED SCENS BUT
%%%%%%%%%%%%%%%%%%%% U=0 IF NOT IN MENU SO DROPPING IT
[m,n]=size(Chat);
...
beta=max(beta,3/2/(s+sfixed)*ysum.*alpha./buffer);
% Build model
...
%%% vfixed and y less x (can only assign if offered) %%%%%%%%%%%%%%%
Anew=sparse(m*n*(s+sfixed),varitot);
Anew(:,xind)=repmat(-speye(m*n),(s+sfixed),1);
...
%%% vfixed less fixed y(can only assign if  accepted) %%%%%%%%%%%%%%%
...
Anew=sparse(m*n,varitot);
Anew(:,yind)=repmat(1/(s+sfixed)*speye(m*n),1,s);
Anew(:,pind)= -sparse(diag(buffer(:)));
ysum=zeros(m,n);

bnew=-1/(s+sfixed)*ysum(:);
...
%%% filter constr, x<=sum_s yhat
Anew=sparse(m*n,varitot);
Anew(:,yind)=repmat(-speye(m*n),1,s);
Anew(:,xind)= speye(m*n);
bnew= sparse(m*n,1);
