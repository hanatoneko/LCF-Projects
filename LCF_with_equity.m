function[obj,gap,v,w,y,p,ptrue,u,psi]=LCF_with_equity(Chat,x,q,s,fyhat,fscenprobs,fare,alpha,beta,premium,mincomp,uconst,rhs,timelim,gap,displ)
%%% code to solve lcf-fr
...
%constraint that the offer is at least $4. Uconst and rhs are the constraint arrays of the 
%fairness constraints produced by create_uconst, timelim is the solver time
...
beta=max(beta,3/2/(s+sfixed)*x.*ysum.*alpha./premium);

%%% set max comp for each driver with i in menu to be the max possible needed comp for any
%%% driver offered request i (see fairness paper beta formula used to
%%% ensure feasibility
for i = 1:m
    MaxCompI = max((beta(i,:)+alpha(i,:)).*x(i,:));
    if MaxCompI > 0
        for j = 1:n
            if x(i,j) == 1
               beta(i,j) = MaxCompI -alpha(i,j);
            end
        end             
    end
end
...
%%% new uconst requiring alpha_ij + u_ij <= alpha_ik + u_ik
if nnz(uconst > 0)
    Anew=sparse(size(uconst,1),varitot);
    Anew(:,uind) = uconst;
    bnew = rhs;
    Am=vertcat(Am,Anew);
    bm=vertcat(bm,bnew);
...
%%% premium (1\samples*sum_s y<= premium*pij)  %%%%%%%%%%%%%%%
Anew=sparse(m*n,varitot);
Anew(:,yind)=repmat(1/(s+sfixed)*diag(x(:)),1,s);
Anew(:,pind)= -sparse(diag(premium(:)));
