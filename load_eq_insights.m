%this code is basically the same code as load_eq_all except this code
%considers only the experiments that were used to get solution insights in the 
%fairness analysis, that is, one premium setting for each fairness type and
%threshold: SLSF, SOL-IT, dp(10,0.875),dp(20,1), ch(3,0.875),ch(10,1),eq(-,1)
...
%data file named 'concat_vals_eq.mat' that contains performance information from every selected experiment by loading each selected experiment
...
%info then dp(10,0.875) and so on
...
variscens_lcf_it = 5;
NumMetrics = 4;
NumExperiments = 7; %number of experiments, the experiments are: SLSF, SOL-IT, dp(10,0.875),dp(20,1), ch(3,0.875),ch(10,1),eq(-,1)
...
guessbestiter_concat = zeros(NumExperiments-1,trials);
...
%v_lcf_it_concat = zeros(m*trials*(NumExperiments),n*(fixedscens_lcf_it + variscens_lcf_it));
...
%0.875. The first experiment is SLSF and the second is SOL-IT and the rest
%are SOL-FR with different settings.
eqtype_concat = ['na';'na';'dp';'dp';'ch';'ch';'eq'];
threshold_concat = [0,0,10,20,3,10,0];
premiumstr_concat = ['na';'na';'87';'10';'87';'10';'10'];
