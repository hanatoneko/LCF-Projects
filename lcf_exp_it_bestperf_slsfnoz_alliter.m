function[] = lcf_exp_it_bestperf_slsfnoz_alliter(iteration_max,premium_diff,premiumstr_diff,variscens_lcf_it_bestperf,fixedscens_lcf_it_bestperf,notes_bestperf)
...
stem = sprintf('lcf_it_noperf_slsfnoz_alliterI%dP%sFS%dVS%d%s.mat',iteration_max,premiumstr_diff,fixedscens_lcf_it_bestperf,variscens_lcf_it_bestperf,notes_bestperf);
...
for t=1:trials
    ...
    if guessbestiter_lcf_it_noperf(1,t) >= 1
        ...
        %%% performance analysis of initial slsfnoz soln
     % generate test scenarios
     tic
    [testyhat,testscenprobs,testscenbase,testscenexp]=indepy_generate_scens(x_slsfnoz(:,(t-1)*n+1:t*n).*yprobslong(:,(t-1)*n+1:t*n),testsamps,0);
    toc
    ...
    tic
    [objave,objsd,dupave,dupsd,rejave,rejsd,aveassign,objvals,vfull,zfull,wfull]=sample_cases_performance_indepy(Clong(:,(t-1)*n+1:t*n),Dlong(:,(t-1)*n+1:t*n),rlong(:,t),qlong,x_slsfnoz(:,(t-1)*n+1:t*n),yprobslong(:,(t-1)*n+1:t*n),5000,testyhat,testscenprobs);
    toc
...
stem = sprintf('lcf_it_bestperf_slsfnoz_allitermaxI%dP%sFS%dVS%d%s.mat',iteration_max,premiumstr_diff,fixedscens_lcf_it_bestperf,variscens_lcf_it_bestperf,notes_bestperf);
