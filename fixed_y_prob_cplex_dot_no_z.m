function[objf,gap,x,v,w]=fixed_y_prob_cplex_dot_no_z(C,D,r,theta,q,yprobs,yhat,scenprobs,equal,lwr,timelim,gap,displ)
%%% formulates and solves SLSFnoZ, i.e. SLSF when we don't model unhappy
%%% drivers
...
%%% scenario, scenarios concatenated horizontally so v is m x n*s),
%%% umached requets: w is the binary values where 1 means
...
%%% concatenated horizontally), scenprobs is a row vector of the scenario liklihoods,
...
ubm=vertcat(yprobsround(:),ones(m*n*s,1));

%%% all variables are binary (B)
ctypem=char(ones([1 m*n+m*n*s])*('B'));
