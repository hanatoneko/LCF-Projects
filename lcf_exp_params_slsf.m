...
%LCF fixed scenarios, generated for SOL-FMS and SOL-IT's first iteration
yhat_lcfinput=zeros(m*trials,n*100);
scenprobs_lcfinput=zeros(trials,100);
scenbase_lcfinput=zeros(trials,100);
scenexp_lcfinput=zeros(trials,100);
runtimes_scens_lcfinput=zeros(1,trials);

for t=1:trials
    %generate and save input parameters
    while true % redo param generation and slsf if 60 seconds isn't long enough to get a decent solution (sometimes slsf times out)
    [Chat,C,D,r,fare,qlong,yprobs,slsfcomp,alphavals,betavals,Oi,Di,Oj,Dj,OjOimins,extradrivingtime,drand]=generate_params_compmodel(m,n,slsfperc);
    ...
    betavalslong(:,(t-1)*n+1:t*n)=betavals;
    Oilong(:,t)=Oi;
    Dilong(:,t)=Di;
    Ojlong(:,t)=Oj;
    Djlong(:,t)=Dj;
    ...
    %generate and save mutated slsf scenarios
    tic
    ...
    % generate LCF fixed scenarios
    tic
    [yhat,scenprobs,scenbase,scenexp]=indepy_generate_scens(x.*yprobs,100,0);
    toc
    yhat_lcfinput((t-1)*m+1:t*m,:)=yhat;
    scenprobs_lcfinput(t,:)=scenprobs;
    scenbase_lcfinput(t,:)=scenbase;
    scenexp_lcfinput(t,:)=scenexp;
    runtimes_scens_lcfinput(1,t)=toc;
